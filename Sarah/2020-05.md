...
J'ai refactorisé ma fonction affichagePath en essayant de suivre les conseils De Laila et Yannick en utilisant les data-attributes. 
...
Laila m'a conseillé une écriture plus moderne pour perfectionner la concaténation de varaiables : 
```js
`function RendreVisible(classPath) {
  document.querySelectorAll(`[data-color="${classPath}"]`).forEach(function(path){
    path.style.visibility="visible";
